...
/**
 * loonggpu_debugfs_add_files - Add simple debugfs entries
 *
			     ...
			     unsigned nfiles)
{
	...
	if (i > LOONGGPU_DEBUGFS_MAX_COMPONENTS) {
		DRM_ERROR("Reached maximum number of debugfs components.\n");
		DRM_ERROR("Report so we increase "
			  "LOONGGPU_DEBUGFS_MAX_COMPONENTS.\n");
		return -EINVAL;
	}
...
/**
 * loonggpu_debugfs_regs_read - Callback for reading MMIO registers
 */
static ssize_t loonggpu_debugfs_regs_read(struct file *f, char __user *buf,
...
/**
 * loonggpu_debugfs_regs_write - Callback for writing MMIO registers
 */
static ssize_t loonggpu_debugfs_regs_write(struct file *f, const char __user *buf,
					 size_t size, loff_t *pos)
{
	struct loonggpu_device *adev = file_inode(f)->i_private;
...
/**
 * loonggpu_debugfs_gca_config_read - Read from gfx config data
 *
 * @f: open file handle
...
static ssize_t loonggpu_debugfs_gca_config_read(struct file *f, char __user *buf,
					size_t size, loff_t *pos)
{
	struct loonggpu_device *adev = file_inode(f)->i_private;
	ssize_t result = 0;
...
/**
 * loonggpu_debugfs_sensor_read - Read from the powerplay sensors
 *
 * @f: open file handle
 * @buf: User buffer to store read data in
 * @size: Number of bytes to read
 * @pos:  Offset to seek to
 */
static ssize_t loonggpu_debugfs_sensor_read(struct file *f, char __user *buf,
					size_t size, loff_t *pos)
{
	return -EINVAL;
}

/** loonggpu_debugfs_wave_read - Read WAVE STATUS data
 *
 * @f: open file handle
 * @buf: User buffer to store read data in
 * @size: Number of bytes to read
 ...
 */
static ssize_t loonggpu_debugfs_wave_read(struct file *f, char __user *buf,
					size_t size, loff_t *pos)
{
	struct loonggpu_device *adev = f->f_inode->i_private;
	int r, x;
	ssize_t result = 0;
...
/** loonggpu_debugfs_gpr_read - Read wave gprs
 *
 * @f: open file handle
 * @buf: User buffer to store read data in
 * @size: Number of bytes to read
 ...
 */
static ssize_t loonggpu_debugfs_gpr_read(struct file *f, char __user *buf,
					size_t size, loff_t *pos)
{
	struct loonggpu_device *adev = f->f_inode->i_private;
	int r;
...
static const struct file_operations loonggpu_debugfs_regs_fops = {
	.owner = THIS_MODULE,
	.read = loonggpu_debugfs_regs_read,
	.write = loonggpu_debugfs_regs_write,
	.llseek = default_llseek
};

static const struct file_operations loonggpu_debugfs_gca_config_fops = {
	.owner = THIS_MODULE,
	.read = loonggpu_debugfs_gca_config_read,
	.llseek = default_llseek
};

static const struct file_operations loonggpu_debugfs_sensors_fops = {
	.owner = THIS_MODULE,
	.read = loonggpu_debugfs_sensor_read,
	.llseek = default_llseek
};

static const struct file_operations loonggpu_debugfs_wave_fops = {
	.owner = THIS_MODULE,
	.read = loonggpu_debugfs_wave_read,
	.llseek = default_llseek
};
static const struct file_operations loonggpu_debugfs_gpr_fops = {
	.owner = THIS_MODULE,
	.read = loonggpu_debugfs_gpr_read,
	.llseek = default_llseek
};

static const struct file_operations *debugfs_regs[] = {
	&loonggpu_debugfs_regs_fops,
	&loonggpu_debugfs_gca_config_fops,
	&loonggpu_debugfs_sensors_fops,
	&loonggpu_debugfs_wave_fops,
	&loonggpu_debugfs_gpr_fops,
};
...
/**
 * loonggpu_debugfs_regs_init -	Initialize debugfs entries that provide
 * 								register access.
 *
...
static int loonggpu_debugfs_test_ib(struct seq_file *m, void *data)
{
...
static int loonggpu_debugfs_test_ring(struct seq_file *m, void *data)
{
...
static int loonggpu_debugfs_test_xdma(struct seq_file *m, void *data)
{
	...
	for (i = 0; i < adev->xdma.num_instances; i++) {
		...
		r = loonggpu_ring_test_xdma(ring, msecs_to_jiffies(5000));
		if (r)
			seq_printf(m, "%s ring tests failed (%d).\n", ring->name, r);
		else
			seq_printf(m, "%s ring tests passed.\n", ring->name);
...
static int loonggpu_debugfs_evict_vram(struct seq_file *m, void *data)
{
	struct drm_info_node *node = (struct drm_info_node *)m->private;
	struct drm_device *dev = node->minor->dev;
	struct loonggpu_device *adev = dev->dev_private;
...
static int loonggpu_debugfs_evict_gtt(struct seq_file *m, void *data)
{
	struct drm_info_node *node = (struct drm_info_node *)m->private;
	struct drm_device *dev = node->minor->dev;
	struct loonggpu_device *adev = dev->dev_private;

	seq_printf(m, "(%d)\n", lg_ttm_resource_manager_evict_all(adev, TTM_PL_TT));
	return 0;
}

static const struct drm_info_list loonggpu_debugfs_list[] = {
	{"loonggpu_vbios", loonggpu_debugfs_get_vbios_dump},
...
{
	return loonggpu_debugfs_add_files(adev, loonggpu_debugfs_list,
					ARRAY_SIZE(loonggpu_debugfs_list));
}
